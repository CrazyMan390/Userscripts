...
switch (storePageResult) {
    ...
    case "ORIGIN":
        ...
        var urlName = docURL.split("/").pop();
        var realName = urlName.replace('-', ' ')
        appName = realName;
        var checkExist = setInterval(function() {
            if ($('.otkex-dropdown-cta').length) {
                console.log("Exists!");
                pirateLinks.forEach((e) => {
                    $(".otkex-dropdown-cta").append('<div class="otkbtn otkbtn-dropdown otkex-dropdown-wrapper" ><button class="otkbtn otkbtn-primary otkbtn-primary-btn" onClick="window.location.href=&#39' + e.url + appName + '&#39;"> ' + e.title + ' </button> </div >');
                })
